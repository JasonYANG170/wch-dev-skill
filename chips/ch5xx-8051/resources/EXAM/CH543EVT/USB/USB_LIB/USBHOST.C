...
* Description    : 检测到设备后,复位总线,为枚举设备准备,设置为默认为低速
* Input          : None   
* Output         : None
* Return         : None
********************************************************************************/
void ResetRootHubPort( void )
{
	...
	SetUsbSpeed( 0 );                                                           /* 默认为低速 */
	UHOST_CTRL = UHOST_CTRL & ~bUH_LOW_SPEED | bUH_BUS_RESET;                   /* 默认为低速,开始复位 */
...
* Description    : CH543传输事务,输入目的端点地址/PID令牌,同步标志,以20uS为单位的NAK重试总时间(0则不重试,0xFFFF无限重试),返回0成功,超时/出错重试
...
/********************************************************************************
* Function Name  : TouchStartAOA( void )
* Description    : AOA模式
* Input          : None
* Output         : None
* Return         : 状态
...
USBDevEnum:
	...
	if( s == ERR_SUCCESS )
	{
		...
		{
			printf("%02X ", (UINT16)(Com_Buffer[i]));				
		}
		printf( "\n" );                                                      	/* 显示出描述符 */
#endif	
		
		/* 保存VID PID信息 */
		ThisUsbDev.DeviceVID = ( ( (UINT16)( (PXUSB_DEV_DESCR)Com_Buffer )->idVendorH ) << 8 ) + ( (PXUSB_DEV_DESCR)Com_Buffer )->idVendorL; 
		...
		{
			/* 保存USB地址 */
			ThisUsbDev.DeviceAddress = ( (PUSB_SETUP_REQ)SetupSetUsbAddr )->wValueL;  
			...
			if( s == ERR_SUCCESS )
			{
				...
				printf("\n");
#endif								
				/* 接口类代码 */																																		//分析配置描述符,获取端点数据/各端点地址/各端点大小等,更新变量endp_addr和endp_size等
				if_cls = ( (PXUSB_CFG_DESCR_LONG)Com_Buffer )->itf_descr.bInterfaceClass;  								
				...
				else if( dv_cls == 0x00 && if_cls == USB_DEV_CLASS_PRINTER && ( (PXUSB_CFG_DESCR_LONG)Com_Buffer )->itf_descr.bInterfaceSubClass == 0x01 )
				{
					/* 是打印机类设备 */
...
//						}
				...
				else if( ( dv_cls == 0x00 ) && ( if_cls == USB_DEV_CLASS_HID ) && ( ( (PXUSB_CFG_DESCR_LONG)Com_Buffer )-> itf_descr.bInterfaceSubClass <= 0x01 ) )
				{
					/* 是HID类设备,键盘/鼠标等 */
			...
			printf("\n");
						...
						/* 保存HUB的端口数量 */						
						ThisUsbDev.GpHUBPortNum = ( (PXUSB_HUB_DESCR)Com_Buffer ) -> bNbrPorts;
									
						if( ThisUsbDev.GpHUBPortNum > HUB_MAX_PORTS )
						{
							/* 因为定义结构DevOnHubPort时人为假定每个HUB不超过HUB_MAX_PORTS个端口 */
...
{
	UINT8 RootHubIndex;                                                         /* CH543只有一个USB口,RootHubIndex = 0,只需看返回值的低八位即可 */
	UINT8 HubPortIndex;
	
	RootHubIndex = 0;
	
	if( ( ThisUsbDev.DeviceType == USB_DEV_CLASS_HUB ) && ( ThisUsbDev.DeviceStatus >= ROOT_DEV_SUCCESS ) )
	{
		/* 外部集线器HUB且枚举成功 */
...
void InitUSB_Host( void )
{
	...
	UH_RX_DMA_L = RxBuffer;
	UH_TX_DMA_L = TxBuffer;
	UH_RX_CTRL = 0x00;
	UH_TX_CTRL = 0x00;
