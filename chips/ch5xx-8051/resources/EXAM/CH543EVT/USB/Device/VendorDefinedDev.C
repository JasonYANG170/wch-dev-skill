...
* Version            : V1.0
* Date               : 2022/01/13
* Description        : CH543模拟USB Module,厂商自定义接口设备，需要安装驱动，
...
UINT8X	Ep2Buffer[ MIN( 64, ENDP2_IN_SIZE + 2 ) + MIN( 64, ENDP2_OUT_SIZE + 2 ) ] _at_ ( MIN( 64, THIS_ENDP0_SIZE + 2 ) + MIN( 64, ENDP1_IN_SIZE + 2 ) );
...
UINT8C	MyDevDescr[ ] = 
{ 
    0x12, 0x01, 0x10, 0x01, 0xFF, 0x80, 0x55, THIS_ENDP0_SIZE,
    0x48, 0x43, 0x37, 0x55, 0x00, 0x01, 0x01, 0x02,
    0x00, 0x01 
};
		...
		switch( USB_INT_ST & ( bUIS_SETUP_ACT | MASK_UIS_TOKEN | MASK_UIS_ENDP ) )  /* 分析操作令牌和端点号 */
        {
			...
			case bUIS_SETUP_ACT | UIS_TOKEN_OUT | 2:
				if ( U_TOG_OK )                                                 /* 不同步的数据包将丢弃 */
                {
					...
					for ( i = 0; i < len; i ++ )
                    {
						Ep2Buffer[ ENDP2_IN_SIZE + i ] = Ep2Buffer[ i ] ^ 0xFF; /* OUT数据取反到IN由计算机验证 */
					}
					UEP2_T_LEN = len;
...
void	InitUSB_Device( void )
{
	...
	USB_CTRL = bUC_DEV_EN | bUC_INT_BUSY;
