...
* Version            : V1.0
* Date               : 2020/07/03
* Description        : CH557 C语言的U盘目录文件枚举程序
 支持: FAT12/FAT16/FAT32
 注意包含 CH557UFI.LIB/CH557_USB.C/DEBUG.C
 注；使用CH557UFI.LIB注意(编译器内存模式设置成:small)
*********************************************************************************
...
void mStopIfError(UINT8 iError)
{
	...
	printf("Error: %02X\n", (UINT16)iError);  /* 显示错误 */
	/* 遇到错误后,应该分析错误码以及CH557DiskStatus状态,例如调用CH557DiskReady查询当前U盘是否连接,如果U盘已断开那么就重新等待U盘插上再操作,
		 建议出错后的处理步骤:
		 1、调用一次CH557DiskReady,成功则继续操作,例如Open,Read/Write等
		 2、如果CH557DiskReady不成功,那么强行将从头开始操作(等待U盘连接，CH557DiskReady等) */
	while(1)
	{
	}
}
...
void main(void)
{
	...
	InitUSB_Host();
	CH549LibInit();                                                          //初始化CH557程序库以支持U盘文件
	FoundNewDev = 0;
