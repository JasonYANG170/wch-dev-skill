...
* Version            : V1.0
* Date               : 2017/3/8
* Description        :本例程用于演示上位机软件通过UDP广播来配置CH561的功能.
                       561创建了一个UDP server来与上位机软件通信
...
void CH561_RESET()
{
    ...
    R8_GLOB_RST_CFG = 0x40 | RB_GLOB_FORCE_RST;                                 /*软件复位*/  
...
int main( void ) 
{
    ...
    if(CFG->cfg_flag[0]!=checkcode1||CFG->cfg_flag[1]!=checkcode2){             /*校验EEPROM里的信息是否合法*/                                                                               /*若EEPROM里保存的信息不合法，或561还未经过上位机配置,那么就按照默认配置来初始化561*/                                                                                                         
      i = EEPROM_ERASE(0x1000,4096);                                                   
      i = EEPROM_WRITE(0x1000,Default_cfg,MODULE_CFG_LEN );                                       
      CH561_RESET();
    }
    i=CH56X_GetMac( MACAddr ); 
    if(i){                                                                      /*如果读取eeprom里的mac失败，那么561就使用默认MAC*/
    ...
    InitSysHal();                                                               /* 初始化中断 */
    CH561NET_CreatUdpSocket();                                                  /* 创建UDP Socket */
    if(CFG->type!=NET_MODULE_TYPE_NONE){                                        /*默认配置下不开启任何模式*/      
    CH561NET_Creat_Communication_Socket();                                      /*创建上位机配置模式下的socket*/
    }  
    if((R32_PB_PIN&(1<<6))==0) {                                                /*按键按下后，初始化561，执行默认配置*/ 
       Delay_ms(50);
       if((R32_PB_PIN&(1<<6))==0){      
