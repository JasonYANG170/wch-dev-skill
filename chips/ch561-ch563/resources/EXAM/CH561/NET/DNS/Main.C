...
* Version            : V1.0
* Date               : 2017/3/8
* Description        : CH561NET库演示
*                    
*******************************************************************************/

/*******************************************************************************
CH561 TCP/IP 协议族接口
CH561NET库演示文件，DNS域名解析功能演示。
...
/* 库配置 */
/* MAC 收发缓冲区配置 */
/* CH561NET.LIB 内部会分配 (RX_QUEUE_ENTRIES + TX_QUEUE_ENTRIES)*MAX_PKT_SIZE 个字节的缓冲区用于以太网的数据收发 */
...
UINT8 IPAddr[4] = {192,168,0,11};                                               /* IP地址 */
UINT8 GWIPAddr[4] = {192,168,0,1};                                              /* 网关 */
UINT8 IPMask[4] = {255,255,255,0};                                              /* 子网掩码 */
UINT8 RemoIP[4] = {192,168,0,3};                                                /* 子网掩码 */
...
/*******************************************************************************
* Function Name  : CH561NET_DNSCallBack
* Description    : DNS回调函数
* Input          : None
* Output         : None
* Return         : 执行状态
*******************************************************************************/
void  CH561NET_DNSCallBack(const  char  *name,  UINT8  *ipaddr,  void  *callback_arg) 
{ 
    if(ipaddr == NULL) 
    { 
        printf("DNS Fail\n"); 
...
int main(void) 
{
    ...
    while(1)
    {
        CH563NET_MainTask();                                                    /* NET库主任务函数，需要在主循环中不断调用 */
        if(CH563NET_QueryGlobalInt())CH561NET_HandleGloableInt();               /* 查询中断，如果有中断，则调用全局中断处理函数 */
        if(DNSGetFlag)
        {
            DNSGetFlag = 0;
            i= CH563NET_GetHostName("www.wch.cn",RemoteIp,CH561NET_DNSCallBack,NULL);
            if(i == 0) 
            { 
    #if CH561NET_DBG
                printf("561NET_GetHostName Success\n"); 
