...
*                      (2)、本程序用于演示TCP 服务端通讯，单片机收到数据后，回传给远端
...
/* CH561NET库TCP的MSS长度为536字节，即一个TCP包里的数据部分最长为536字节 */
...
/* CH561相关定义 */
UINT8 MACAddr[6] = {0x02,0x03,0x04,0x05,0x16,0x17};                             /* MAC地址 */
UINT8 IPAddr[4] = {192,168,1,2};                                                /* IP地址 */
UINT8 GWIPAddr[4] = {192,168,1,1};                                              /* 网关 */
...
* Description    : 系统定时器初始化，100MHZ TIME0 10ms，根据NETTIMEPERIOD
*                ：来初始化定时器。
* Input          : None
* Output         : None
* Return         : None
...
void CH561NET_HandleSockInt(UINT8 sockeid,UINT8 initstat)
{
    ...
    {
#if CH561NET_DBG                                                                /* 产生此中断，NET库内部会将此socket清除，置为关闭*/
        printf("TCP Timout\n");                                                 /* 应用曾需可以重新创建连接 */
#endif
    }
}
...
/*******************************************************************************
* Function Name  : CH561NET_CreatTcpSocket
...
void CH561NET_CreatTcpSocket(void)
{
   UINT8 i;                                                             
...
/*******************************************************************************
* Function Name  : CH561NET_DHCPCallBack
* Description    : DHCP回
* Input          : None
* Output         : None
* Return         : 执行状态
*******************************************************************************/
UINT8 CH561NET_DHCPCallBack(UINT8 status,void *arg)
{
   UINT8 *p;
   ...
   if(!status){                                                                 /* 成功*/
       ...
       printf("DNS2: %d.%d.%d.%d\n",p[16],p[17],p[18],p[19]);
#endif 
       CH561NET_CreatTcpSocket();                                               /* 创建TCP Socket */
   }
   else{
#if CH561NET_DBG                                                                /* 产生此中断，NET库内部会将此socket清除，置为关闭*/
...
#endif 
    if(!i){
        CH563NET_DHCPStart(CH561NET_DHCPCallBack);                              /* 启动DHCP */
    }
    SysTimeInit();                                                              /* 系统定时器初始化 */
    InitSysHal();                                                               /* 初始化中断 */
    while(1){
        CH563NET_MainTask();                                                    /* 库主任务函数，需要在主循环中不断调用 */
