...
* Version            : V1.0
* Date               : 2017/3/8
* Description        : CH561网络部分初始化及MAC收发接口函数
*******************************************************************************/
...
MACCtrl      mac_cotrl;                                                         /* CH561MAC控制结构 */
...
/*******************************************************************************
* Function Name  : CH561PHY_Process
* Description    : PHY处理函数，根据协商结果，来配置MACCR寄存器
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void CH561PHY_Process(void)
{
    UINT16 value;
...
/*******************************************************************************
* Function Name  : CH561MAC_ParamInit
* Description    : 参数初始化函数，初始化全局变量
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void CH561MAC_ParamInit(void)
{
...
/*******************************************************************************
* Function Name  : CH561MAC_Open
* Description    : CH561MAC打开函数，本函数使用默认配置
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void CH561MAC_Open(void)
{
    ...
    i = (UINT32)(CH561MACAddr[2]<<24) + (UINT32)(CH561MACAddr[3]<<16) + (UINT32)(CH561MACAddr[4]<<8) + CH561MACAddr[5]; 
    R32_ETHE_MAC_LADR = i;                                                      /* 写MAC地址寄存器的低4节*/
    i = (UINT32)(CH561MACAddr[0]<<8) + CH561MACAddr[1]; 
...
/*******************************************************************************
* Function Name  : CH561NET_Init
* Description    : 初始化CH56以太网部分
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void CH561NET_Init(void)
{
...
void NET_INIT( void )
{
    CH561MAC_ParamInit( );                                                      /* 变量初始化 */
    CH561NET_Init( );                                                           /* 开启以太网部分电源，时钟，elink灯 */
    CH561MAC_Open( );                                                           /* 初始化MAC */
    CH561PHY_Process( );                                                        /* 初始化PHY */
}
...
* Description    : 由CH561MAC_Isr调用，CH561MAC发送完成中断
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
* Description    : 由CH561MAC_Isr调用，接收完毕中断处理函数
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void MAC_RecvFinishHanld(void)
{
    ...
    while(1){
        ...
        mac_cotrl.rxdes_cur->rxdes0 = RXDES0_RXDMA_OWN;                         /* 释放DMA，用于多个描述符，否则在CH561MAC_RxSuccessHandle已经跳转 */
        mac_cotrl.rxdes_cur = (mac_rxdes *)mac_cotrl.rxdes_cur->rxdes3;         /* 将当前描述符指向下一个 */
    }
}
...
UINT8 MAC_Output(UINT8 *buf,UINT16 *len,UINT16 code)
{
    ...
    memcpy( ETH.Head.SourMAC,CH561MACAddr,MACADDR_LEN );                        /* 填写本机mac地址 */
    ...
    while(SendSuc){                                                             /* 发送完毕后，会产生发送成功中断CH561MAC_INT_XPKT_OK */
       timecount++;
       Delay_us(10);
...
UINT8 ARP_Output( UINT8 *desip,UINT8 code )
{
    ...
    memcpy(ARP.Package.SENDIP,CH561IPAddr,IPADDR_LEN);                          /* 本机ip地址 */
    memcpy(ARP.Package.SENDMAC,CH561MACAddr,MACADDR_LEN);                       /* 本机mac地址 */
...
void ARP_Input( UINT8 *arpdata,UINT16 len )
{
    ...
    if(    ARP.Package.HRDTYPE == GLOB_nTohs( ARP_HARDWARE_TYPE_ETHER ) &&
        ...
        if( ARP.Package.OPCODE  == GLOB_nTohs( ARP_CODE_QUEST )){
            if(  memcmp(ARP.Package.TARGIP,CH561IPAddr,IPADDR_LEN) == 0 ){
