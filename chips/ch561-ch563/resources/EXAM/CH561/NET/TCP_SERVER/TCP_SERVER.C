...
#define CH561NET_DBG                           1                                /* printf调试使能 */
...
const UINT8 IPAddr[4] = {192,168,0,2};                                          /* IP地址 */
...
void CH561NET_HandleSockInt(UINT8 sockeid,UINT8 initstat)
{
    ...
    if(initstat & SINT_STAT_CONNECT)                                            /* TCP连接中断 */
    {
        ...
        /* 在TCP SERVER模式下，如果检测到TCP连接，则CH561NET将会自动为      */
...
int main( void ) 
{
    ...
    mStopIfError(i);                                                            /* 检查错误 */
#if CH561NET_DBG
    printf("CH561NETLibInit Success\n");
#endif    
    ...
    while(1)
    {
        CH563NET_MainTask();                                                    /* CH561NET库主任务函数，需要在主循环中不断调用 */
