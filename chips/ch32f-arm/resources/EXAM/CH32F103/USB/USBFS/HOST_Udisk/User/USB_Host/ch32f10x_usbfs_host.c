...
/*******************************************************************************/
/* Variable Definition */
__attribute__((aligned(4))) uint8_t  USBFS_RX_Buf[ MAX_PACKET_SIZE ];           // IN, must even address
__attribute__((aligned(4))) uint8_t  USBFS_TX_Buf[ MAX_PACKET_SIZE ];           // OUT, must even address
...
void USBFS_Host_Init( FunctionalState sta , PWR_VDD VDD_Voltage)
{
    ...
    if( sta )
    {
        ...
        R16_UH_RX_DMA = (uint16_t)(uint32_t)USBFS_RX_Buf;
...
uint8_t USBFSH_CtrlTransfer( uint8_t ep0_size, uint8_t *pbuf, uint16_t *plen )
{
    ...
    Delay_Us( 100 );
    s = USBFSH_Transact( ( R8_UH_TX_LEN ? USB_PID_IN << 4 | 0x00: USB_PID_OUT << 4 | 0x00 ), RB_UH_R_TOG | RB_UH_T_TOG, DEF_CTRL_TRANS_TIMEOVER_CNT );  /* STATUS阶段 */
    if( s != ERR_SUCCESS )
    {
        return s;
    }
