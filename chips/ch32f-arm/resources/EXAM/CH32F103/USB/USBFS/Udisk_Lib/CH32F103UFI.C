/********************************** (C) COPYRIGHT *******************************
 * File Name          : CH32F103UFI.C
 * Author             : WCH
 * Version            : V1.0.0
 * Date               : 2019/10/15
 * Description        : USB-flash File Interface for CH103
...
{
    uint8_t ret;
    ret = USBFSH_GetDeviceDescr( &RootHubDev.bEp0MaxPks, DataBuf );
    //printf( "02 ret %02x\r\n", ret );
    return ret;
}
...
UINT8 CtrlGetConfigDescr( PUINT8 DataBuf )
{
    uint16_t len;
    uint8_t  ret;
    ret = USBFSH_GetConfigDescr( RootHubDev.bEp0MaxPks, DataBuf, DEF_COM_BUF_LEN, &len );
    //printf( "03 ret %02x\r\n", ret );
    return ret;
}
...
UINT8	CH103ReadSector( UINT8 SectCount, PUINT8 DataBuf )  /* 从磁盘读取多个扇区的数据到缓冲区中 */
{
	UINT8	retry;
//	if ( use_external_interface ) return( extReadSector( CH103vLbaCurrent, SectCount, DataBuf ) );  /* 外部接口 */
	for( retry = 0; retry < 3; retry ++ ) {  /* 错误重试 */
		pCBW -> mCBW_DataLen = (UINT32)SectCount << CH103vSectorSizeB;  /* 数据传输长度 */ 
		pCBW -> mCBW_Flag = 0x80;
		...
		CH103BulkOnlyCmd( DataBuf );  /* 执行基于BulkOnly协议的命令 */
	...
	return( CH103IntStatus = ERR_USB_DISK_ERR );  /* 磁盘操作错误 */
}

#ifdef	EN_DISK_WRITE
UINT8	CH103WriteSector( UINT8 SectCount, PUINT8 DataBuf )  /* 将缓冲区中的多个扇区的数据块写入磁盘 */
{
	UINT8	retry;
//	if ( use_external_interface ) return( extWriteSector( CH103vLbaCurrent, SectCount, DataBuf ) );  /* 外部接口 */
	for( retry = 0; retry < 3; retry ++ ) {  /* 错误重试 */
		pCBW -> mCBW_DataLen = (UINT32)SectCount << CH103vSectorSizeB;  /* 数据传输长度 */
		pCBW -> mCBW_Flag = 0x00;
		pCBW -> mCBW_LUN = CH103vCurrentLun;
...
mDiskDisconn:
		...
		if ( ( R8_USB_DEV_AD & 0x20 ? UHUB1_CTRL : UHUB0_CTRL ) & RB_UH_PORT_EN ) {  /* 内置Root-HUB下的外部HUB存在且未插拔 */
			...
			CH103IntStatus = HubGetPortStatus( TxBuffer[ MAX_PACKET_SIZE - 1 ] & 0x0F );  /* 查询HUB端口状态,返回在TxBuffer中 */
			if ( CH103IntStatus == ERR_SUCCESS ) {
				if ( TxBuffer[2] & (1<<(HUB_C_PORT_CONNECTION-0x10)) ) {  /* 检测到HUB端口上的插拔事件 */
					CH103DiskStatus = DISK_DISCONNECT;  /* 假定为HUB端口上的USB设备断开 */
				...
				if ( CH103IntStatus == ERR_USB_DISCON ) {
					...
					CH103DiskStatus = DISK_CONNECT;  /* HUB操作失败 */
					return( CH103IntStatus );
				}
			}
		}
...
{
/* 如果指定枚举序号CH103vFileSize为0xFFFFFFFF后调用FileOpen，那么每搜索到一个文件FileOpen都会调用本回调程序，
   回调程序xFileNameEnumer返回后，FileOpen递减CH103vFileSize并继续枚举直到搜索不到文件或者目录。建议做法是，
   在调用FileOpen之前定义一个全局变量为0，当FileOpen回调本程序后，本程序由CH103vFdtOffset得到结构FAT_DIR_INFO，
   ...
   如果在本回调程序xFileNameEnumer中将CH103vFileSize置为1，那么可以通知FileOpen提前结束搜索。以下是回调程序例子 */
#if		0
	UINT8			i;
	UINT16			FileCount;
	PX_FAT_DIR_INFO	pFileDir;
	PUINT8			NameBuf;
	pFileDir = (PX_FAT_DIR_INFO)( pDISK_BASE_BUF + CH103vFdtOffset );  /* 当前FDT的起始地址 */
	FileCount = (UINT16)( 0xFFFFFFFF - CH103vFileSize );  /* 当前文件名的枚举序号,CH103vFileSize初值是0xFFFFFFFF,找到文件名后递减 */
...
UINT8	CH103LibInit( void )  /* 初始化CH103程序库,操作成功返回0 */
{
	if ( CH103GetVer( ) < CH103_LIB_VER ) return( 0xFF );  /* 获取当前子程序库的版本号,版本太低则返回错误 */
#if		DISK_BASE_BUF_LEN > 0
...
#endif
	CH103DiskStatus = DISK_UNKNOWN;  /* 未知状态 */
	CH103vSectorSizeB = 9;  /* 默认的物理磁盘的扇区是512B */
	CH103vSectorSize = 512;  // 默认的物理磁盘的扇区是512B,该值是磁盘的扇区大小
	CH103vStartLba = 0;  /* 默认为自动分析FDD和HDD */
    CH103vPacketSize = 64;  /* USB存储类设备的最大包长度:64@FS,512@HS/SS,由应用程序初始化,枚举U盘后如果是高速或者超速那么及时更新为512 */
